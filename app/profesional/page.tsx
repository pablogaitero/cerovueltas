...
export default async function ProfesionalDashboard() {
  ...
  // Si aún no tiene perfil profesional creado, redirigir a completar perfil
  ...
  // Últimas conexiones / clientes
  const { data: conexiones } = await supabase
    .from('conexiones')
    .select(`
      id, estado, created_at, monto,
      profiles!conexiones_cliente_id_fkey ( nombre, apellido, empresa, avatar_url )
    `)
    .eq('profesional_id', profesional.id)
  ...
  const totalMes = ingresosMes?.reduce((acc, c) => acc + (c.monto ?? 0), 0) ?? 0
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">
              {profesional.disponible
                ? 'Visible para clientes — recibirás nuevas conexiones'
                : 'Oculto — no aparecerás en búsquedas'}
            </p>
          </div>
          <DisponibilidadToggle
            profesionalId={profesional.id}
            disponibleInicial={profesional.disponible}
          />
        </div>
          ...
          <div className="col-span-3 bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            {!conexiones?.length ? (
              <div className="py-12 text-center">
                ...
                <Link href="/profesional/perfil" className="btn-gold inline-block mt-3 text-xs px-4 py-2">
                  Completar perfil
                </Link>
              </div>
            ) : (
              ...
              <div className="divide-y divide-gray-50">
                {reviews.map((r: any, i: number) => (
                  <div key={i} className="px-5 py-3">
                    ...
                    {r.comentario && (
                      <p className="text-gray-600 text-xs line-clamp-2 leading-relaxed">"{r.comentario}"</p>
                    )}
        ...
        <div className="bg-white rounded-xl border border-gray-100 px-6 py-5">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {!profesional.especialidades?.length && (
              <p className="text-gray-400 text-xs">Sin especialidades configuradas.</p>
            )}
          </div>
        </div>
