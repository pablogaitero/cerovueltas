...
export default async function ClientesPage() {
  ...
  const { data: conexiones } = await supabase
    .from('conexiones')
    .select(`
      id, estado, monto, created_at, pagado_at,
      profiles!conexiones_cliente_id_fkey (
        id, nombre, apellido, empresa, email, telefono, avatar_url
      )
    `)
  ...
  const totalActivos  = conexiones?.filter(c => c.estado === 'activa').length  ?? 0
  const totalIngresos = conexiones?.filter(c => c.estado === 'activa')
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <p className="text-2xl font-bold text-navy">{conexiones?.length ?? 0}</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          {!conexiones?.length ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-400 text-xs mt-1">
                Completa tu perfil y aparecerás en los resultados de búsqueda.
              </p>
            </div>
          ) : (
                    ...
                    <tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Link
                            href={`/profesional/mensajes?conexion=${c.id}`}
                            className="p-1.5 rounded-lg text-gray-400 hover:text-navy hover:bg-navy/8 transition-colors"
                            title="Enviar mensaje"
                          >
                            <MessageSquare size={15} />
                          </Link>
                          {c.estado === 'pendiente' && (
                            <AceptarConexionBtn conexionId={c.id} />
                          )}
                        </div>
                      </td>
                    </tr>
...
// Inline client component para aceptar conexión
function AceptarConexionBtn({ conexionId }: { conexionId: string }) {
  return (
    <form action={async () => {
      'use server'
      const { createClient: createSrv } = await import('@/lib/supabase/server')
      const supabase = createSrv()
      await supabase.from('conexiones').update({ estado: 'activa' }).eq('id', conexionId)
    }}>
      <button
        type="submit"
